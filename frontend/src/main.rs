    ...
    let get_users = {
        ...
        Callback::from(move |_| {
            ...
            spawn_local(async move {{
                match Request::get("http://127.0.0.1:8000/api/users")
                                .send()
                                .await{
                    Ok(resp) if resp.ok() =>{
                        let fetched_user: Vec<User> = resp.json().await.unwrap_or_default();
                        users.set(fetched_user);
                    }
                    _ => {
            ...
            spawn_local(async move {
                ...
                match response { 
                    Ok(resp) if resp.ok() => {
                        ...
                        message.set("User cSreated Successfully".into());
                        get_users.emit(());
                    }
                    _ => {
                        message.set("Failed to create user".into());
                    }
                }
